...
plotExpression(sce.pbmc,
    features = rownames(sce.pbmc)[659],
...
De los primeros genes del cluster 8 Gráfica, y de los primeros 10, que están subexpresados
...
sce.pbmc_negative <- markers.pbmc[[chosen]][order(markers.pbmc[[chosen]]$summary.logFC,decreasing = T),]

plotExpression(sce.pbmc, rownames(sce.pbmc_negative)[1:4],
    x = "cluster", colour_by = "cluster"
)
...
interesting.up3 <- interesting.up3[interesting.up3$p.value<0.05,]
interesting.up3 <- interesting.up3[interesting.up3$FDR<0.05,]

logFCs <- as.matrix(interesting.up3[, -(1:3)])
colnames(logFCs) <- sub("logFC.", "", colnames(logFCs))
pheatmap(logFCs, breaks = seq(-5, 5, length.out = 101))
```
