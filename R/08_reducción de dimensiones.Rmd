---
title: "08 Reducción de dimensiones"
...
Con los spikes in, los ERCC
- ¿Cómo se están obteniendo los HGVs?
Con los que poseen la variación biológica más importante.
...
- ¿Cómo se está modelando la relación media varianza?
Se modela ajustando la varianza a un modelo de Poisson. 
- ¿Cómo se están obteniendo los HGVs?
Me quedo con el 10% de los que están más alejados de la media. 
...
library(PCAtools)
percent.var <- attr(reducedDim(sce.pbmc), "percentVar")
chosen.elbow <- PCAtools::findElbowPoint(percent.var)
plot(percent.var, xlab = "PC", ylab = "Variance explained (%)")
abline(v = chosen.elbow, col = "red")

#Basado en la estructura de la población 
choices <- getClusteredPCs(reducedDim(sce.pbmc))
chosen.clusters <- metadata(choices)$chosen
plot(choices$n.pcs, choices$n.clusters,
    xlab = "Number of PCs", ylab = "Number of clusters"
)
...
#####Juntando todo para el ejercicio
...
plotReducedDim(sce.zeisel,
    dimred = "PCA",
    ncomponents = 6, colour_by = "level1class"
)
```
