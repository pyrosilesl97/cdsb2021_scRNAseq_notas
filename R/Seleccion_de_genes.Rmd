...
str(hvg.pbmc.cv2.3)
```
#En este caso, todos los pasos posteriores son más lentos. 

```{r}
# Visualicemos la relación con la media
fit.cv2.pbmc <- metadata(dec.cv2.pbmc)

a <- fit.cv2.pbmc$mean[row.names(data.frame(fit.cv2.pbmc$cv2))%in%hvg.pbmc.cv2]

b <- fit.cv2.pbmc$cv2[row.names(data.frame(fit.cv2.pbmc$cv2))%in%hvg.pbmc.cv2]


plot(fit.cv2.pbmc$mean, fit.cv2.pbmc$cv2,
    log = "xy",group_by='fit.cv2.pbmc'
)
curve(fit.cv2.pbmc$trend(x),
    col = "dodgerblue",
    add = TRUE, lwd = 2
)
points(fit.cv2.pbmc$mean[hvg.pbmc.cv2], fit.cv2.pbmc$cv2[hvg.pbmc.cv2])
## Así lo hice, pero era un vector nombrado, entonces no había problema con pedirlos directamente
...
datos <- data.frame(fit.cv2.pbmc$cv2)
datos <- data.frame(row.names(datos),fit.cv2.pbmc$mean,fit.cv2.pbmc$cv2)

datos_grafica <- datos[hvg.pbmc.cv2%in%datos$row.names.datos.,]
```
